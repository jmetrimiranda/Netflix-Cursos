...
### ADR-014: Rate limiting in-memory por instância (sem Redis externo)
**Contexto:** F5 exige rate limit em `/api/checkout/create`, `/api/enrollment`, `/api/auth/callback/credentials` e `/api/contact`. Avaliei Upstash Redis (free tier 10k cmd/dia) vs in-memory.
**Decisão:** Implementação in-memory simples em `src/lib/rate-limit.ts` (token bucket por chave `IP+rota`, `Map<string, { count, resetAt }>`). Sem dependência externa, sem variável de ambiente nova. Quando o app for escalar pra múltiplas instâncias Vercel (não é o caso na escala da Ativa), trocar pra Upstash é um adapter — interface é simples (`limit(key, maxRequests, windowMs) → { ok, retryAfter }`).
**Justificativa:** Volume estimado da Ativa (~20–100 sessões simultâneas no pico, ~50–200 visitas/mês institucionais) cabe folgadamente numa única lambda Vercel. In-memory é zero-config, zero-custo, zero-latência. Vercel mantém warm instances curtas — entre cold starts o limiter "esquece", mas isso é aceitável para o caso de uso (proteção contra abuso, não contra DDoS distribuído).
**Consequências:** Em deploy multi-região ou auto-scaling, cada instância tem seu próprio bucket — atacante poderia distribuir reqs entre instâncias. Pra Ativa hoje (1 região, baixo volume) é um trade-off OK. Documentado claramente no `src/lib/rate-limit.ts`. Quando virar problema, migrar pra Upstash sem mudar callers.

### ADR-015: Recovery de acesso por email+CPF (ponte sem auth)
**Contexto:** Sessão de validação em produção (02/05/2026) revelou um buraco de UX no fluxo paywall do ADR-012. Identificação do aluno é por email salvo em localStorage (`ativa_engenharia_email`); ao acessar de outro navegador / outra máquina / aba anônima, o localStorage está vazio e a página do curso mostra apenas `[Comprar acesso]`, mesmo quando o aluno já tem `Enrollment` ativo. O backend tem proteção contra cobrança duplicada (`/api/checkout/create` retorna 409 e redireciona se reconhece o email), mas o aluno só descobre isso depois de preencher o form de compra — pode desistir antes.
**Decisão:** Botão secundário `[Já comprei? Entrar]` sempre visível ao lado do CTA primário em `/cursos/[slug]`, levando pra `/cursos/[slug]/entrar` (form de recovery com email + CPF). `POST /api/enrollment/recover` faz lookup case-insensitive em `Enrollment` pelo email+courseSlug, compara CPF normalizado (só dígitos) com o `Payment.studentCpf` associado, e se tudo bate retorna `{ ok: true, redirectTo: <primeira aula> }`. Frontend salva o email no localStorage (mesma chave `ativa_engenharia_email`) e roteia. Rate limit 5 reqs/min/IP, mesma camada do ADR-014. Todos os cenários de falha (`not_found`, `not_active`, `cpf_mismatch`) usam a mesma mensagem genérica no frontend pra não vazar quais emails existem no banco.
**Trade-offs aceitos:** O par email+CPF é trivialmente compartilhável — uma vez que o aluno divulga os dois, qualquer pessoa pode acessar o curso indefinidamente em qualquer navegador. Não há proteção contra pirataria deliberada. Isso é equivalente ao baseline atual (compartilhar o localStorage já era trivial), mas reduz fricção UX no caminho legítimo. A "verificação" de CPF não tem valor anti-fraude — serve como segundo dado correlato pra evitar acesso por alguém que descobriu o email sem ter participado da compra.
**Alternativas consideradas:**
- **Magic link por email:** elimina compartilhamento de credencial estática mas exige Resend funcional (já temos em F5) e adiciona latência (aluno espera email). Complexidade de estado/token. Descartado por hora porque a pirataria não é problema mensurável e a UX de "checar email" também tem fricção.
- **Login com senha:** força auth real, mas exige onboarding (criar/recuperar senha) e quebra o modelo "compra é o cadastro" do ADR-012. Descartado por mudança de escopo grande pra problema pequeno.
- **Aceitar a UX ruim atual:** rejeitado — aluno desiste no `[Comprar acesso]` achando que vai pagar 2x.
**Quando reabrir:** se pirataria virar problema mensurável ou houver pedido explícito do cliente. Sinais qualitativos a monitorar (sem threshold absoluto — não temos baseline ainda):
- **Reclamações diretas de alunos** ("comprei o curso e outras pessoas estão acessando com meus dados") via WhatsApp / `/contato`.
